...
approved. `{{CALL:spec}} lane status <name>` says which it is. Fix either in {{CALL:mf-plan}},
...
1. Implement the smallest change that satisfies the task, following patterns already in the
   ...
   `{{CALL:spec}} context <name> --task <id>` writes the derived packet to scratch: the full
...
4. If the check failed twice with materially different approaches, the cause is a design
   ...
   `{{CALL:spec}} finding record <name> --file <finding.json>` with the `cause`, the `tasks` it
   ...
   review: `{{CALL:spec}} finding resolve <name> --id F-1 --review R-2 --remedy "<what>"`.
   Never tick a blocked task.
5. If the remedy needs the contract itself to change, stage it instead of editing it:
   `{{CALL:spec}} amend propose <name> --file <candidate.json>` (`locator`, `equivalent-check`
   or `scope`) holds only the affected tasks while the approved text stays in force, and
   `{{CALL:spec}} amend apply <name> --id A-1` publishes it once its authority is recorded,
...
5. If a host reported what the work cost, import it rather than estimating it:
   `{{CALL:spec}} usage import <name> --file <event.json>`. Failed and abandoned attempts are
