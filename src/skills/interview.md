...
- Skip (go straight to {{CALL:mf-plan}} or {{CALL:execute}}) when the request already names the
...
<!-- /MY-FLOW:CLAUDE -->
<!-- MY-FLOW:CODEX -->
Ask in plain text: one question, then stop and wait. Offer two to four concrete options
when a choice is bounded. Do not proceed to the next round until the user answers.
<!-- /MY-FLOW:CODEX -->
...
1. Unless `--change` was given, create the change with `{{CALL:spec}} new <name>`
...
4. Run `{{CALL:spec}} stage <name> interview` to record the current stage.
...
Next: {{CALL:mf-plan}} <name>   (or {{CALL:execute}} <name> if the design is trivial)
```
