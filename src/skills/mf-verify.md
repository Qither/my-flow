...
2. Open a version-bound attempt: `{{CALL:spec}} evidence begin <name> [--inputs <file>]`. It
...
3. Build the final-verifier packet: `{{CALL:spec}} context <name>` with no `--task`. It carries
...
6. Record the result against the attempt, whatever it is:
   `{{CALL:spec}} evidence record <name> --attempt <n> --result <result.json> --report <file>`.
...
8. Relay the verdict verbatim. If FAIL or INCOMPLETE, list the gaps as the next tasks; do
   not soften the verdict. `{{CALL:spec}} evidence status <name>` says whether the change is
