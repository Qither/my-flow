# my-flow v{{VERSION}} - working agreement

my-flow is a thin workflow layer on top of the native features of this tool. It adds no runtime:
just a four-stage flow, a set of skills, a few subagent roles, and OpenSpec as the durable
record of intent. Everything below is a convention, not a state machine.

## 1. Operating principles

- Work directly by default. Delegate to a subagent only when it materially improves quality,
  speed, or independence of judgment (review, verification, second opinion).
- Evidence over assumption. Read the code before describing it; run the command before
  claiming its result. Newer evidence overrides older hypotheses.
- Ask only when the next step is irreversible, credential-gated, production-facing, or would
  materially change scope. Otherwise choose the safest reasonable interpretation and proceed.
- Surgical diffs: every changed line traces back to the request. No drive-by refactors, no
  new dependencies, no new abstractions unless asked.
- Check official documentation before using an unfamiliar SDK, engine API, or CLI flag.

## 2. The four-stage flow

`interview -> plan -> run -> verify`. Each stage consumes the previous stage's artifact.

| Request looks like | Stages to use |
|---|---|
| Concrete, single file, clear acceptance | `run` only (or just do it) |
| Concrete, multi-file or multi-module | `plan -> run -> verify` |
| Vague, no acceptance criteria, or "should we..." | `interview -> plan -> run -> verify` |
| Touches build config, shaders, engine modules, migrations, auth | never skip `plan` and `verify` |

Rules:
- `interview` asks one question per round and stops when non-goals and decision boundaries
  are explicit. Do not re-open the interview inside `plan`.
- `plan` never implements. `run` never redesigns; if the design is wrong, stop and go back.
- `verify` runs in a separate context from the one that wrote the code.
- Skills: {{CALL:interview}}, {{CALL:plan}}, {{CALL:run}}, {{CALL:verify}}, {{CALL:ask}},
  {{CALL:learn}}, {{CALL:spec}}. When the user says "interview me", "plan this", "run the
  change", "verify", "ask codex/claude", or "make this a skill", use the matching skill.

## 3. OpenSpec conventions (the intent layer)

- `openspec/specs/<capability>/spec.md` is the current truth about behavior.
- `openspec/changes/<name>/` holds the intent for one change: `proposal.md` (why, what,
  non-goals, decision boundaries), `design.md` (how), `tasks.md` (ordered checklist).
- Delta-first: write specs only for behavior that this change adds or modifies.
- `tasks.md` checkboxes (`- [ ] 1.1 ...`) are the only progress ledger. Tick a box only after
  that task's own verification passed. Never keep progress anywhere else.
- `design.md` must contain these two sections, even if the answer is "none":
  - `## Do-Not-Touch` - modules, directories, or files this change must not modify.
  - `## Rebuild / Re-run After Change` - what must be regenerated, rebuilt, cooked, or re-run
    after the edit (project files, build targets, caches, test suites).
- When every box is ticked and `verify` passed, archive the change (`openspec archive <name>`)
  so the delta specs merge into `openspec/specs/`.
- Without the OpenSpec CLI, the same structure still applies; `{{CALL:spec}} new <name>` creates
  the three files from templates. The simplified fallback is one `docs/changes/<name>.md` with
  the same sections.

## 4. Tool-specific notes

<!-- MY-FLOW:CLAUDE -->
Claude Code is the primary interactive executor.
- Agent teams are enabled via `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` in `settings.json`
  (`env`). On Windows teammates run in-process only; do not ask for split panes.
- One loop authority per session: at most one active `/goal` and at most one team. `run`
  prints the `/goal` statement for the user to paste; it never tries to set it itself.
- Start a team only when `design.md` lists two or more disjoint file-ownership groups.
  Teammates edit only files they own and report back through the shared task list.
- For risky changes, work in a worktree (`claude -w <name>`), or give a subagent
  `isolation: worktree`.
- Subagent roles provided by this plugin: `my-flow:planner`, `my-flow:architect`,
  `my-flow:critic`, `my-flow:verifier`. Architect, critic, and verifier are read-only.
<!-- /MY-FLOW:CLAUDE -->
<!-- MY-FLOW:CODEX -->
Codex is an advisor and cross-verifier in this workflow, not the primary executor.
- Default to reviewing, planning, and verifying. Implement only when the user explicitly
  invokes `$my-flow-run` or asks for code changes.
- Do not create goals unless the user invokes `$my-flow-run`. One goal per thread; ask the
  user to clear a stale goal in the UI rather than working around it.
- Native subagents `planner`, `architect`, `critic`, `verifier` live in `~/.codex/agents/`.
  Treat architect, critic, and verifier as read-only: they report, they do not edit.
- No tmux, no team runtime, no state daemons are required or expected.
<!-- /MY-FLOW:CODEX -->

## 5. Verification and completion

- No fresh evidence means not complete. "Should work", "probably passes", and remembered
  results from earlier in the session are not evidence.
- Placeholder TODOs, `test.skip` / `.only`, stub returns, and unimplemented branches are
  blockers, not progress. Inspect changed files for them before claiming completion.
- The writer and the verifier are separate passes. Never approve work in the same context
  that produced it.
- Final gate, in order: (1) targeted verification of the change, (2) cleanup of your own
  diff only, (3) re-verify, (4) independent review ({{CALL:verify}} or {{CALL:ask}}),
  (5) declare done and tick the last box.

## 6. Cross-model rules

- Use {{CALL:ask}} for a second opinion on a design, a review of a diff before the final
  gate, a tie-break when plan review stalls, or unfamiliar API behavior. Never for
  implementation.
- The other model runs read-only. Its output is advice, recorded under `.my-flow/ask/`.
- Disagreements are resolved by evidence or by the user, never by majority.

## 7. Paths

- `.my-flow/` is disposable scratch: `ask/`, `interviews/`, `verify/`, `learned/`, `state/`.
  It is git-ignored and may vanish with a worktree; nothing authoritative lives there.
- Durable intent lives only in `openspec/` (committed) and in promoted skills under
  `.claude/skills/` and `.agents/skills/` (committed).
